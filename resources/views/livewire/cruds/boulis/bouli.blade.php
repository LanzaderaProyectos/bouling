            ...
            <div class="p-6 text-gray-900">
                ...
                <div class="flex justify-between">
                    ...
                    @if (Auth::user()->hasRole('super_admin') || Auth::user()->hasRole('brand'))
                        <div id="roles_list_table_filter" class="row py-3">
                            <div class="col-md-4 col-12">
                                <a href="{{ route('bouli.edit-or-create') }}">
                    ...
                    <table>
                        <thead>
                            <tr class="text-uppercase">
                                ...
                                <th style="width: 10%">Social network</th>
                                <th style="width: 10%">Brand</th>
                                <th style="width: 10%">Date start</th>
                                <th style="width: 10%">Date finish</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($boulis as $bouli)
                                <tr>
                                        ...
                                        <td class="border px-4 py-2">
                                            <div class="flex">
                                                <div class="mr-1">
                                                    ...
                                                    <a href="{{ route('bouli.edit-or-create', $bouli) }}">
                                    ...
                                    @endif
                                    <td class="border px-4 py-2">{{ $bouli->name ?? '' }}</td>
                                    <td class="border px-4 py-2">{{ $bouli->requirement ?? '' }}</td>
                                    <td class="border px-4 py-2">{{ $bouli->reward ?? '' }}</td>
                                    <td class="border px-4 py-2">{{ $bouli->social_network ?? '' }}</td>
                                    <td class="border px-4 py-2">{{ $bouli->brand->legal_name ?? '' }}</td>
                                    <td class="border px-4 py-2">{{ $bouli->date_start ?? '' }}</td>
                                    <td class="border px-4 py-2">{{ $bouli->date_finish ?? '' }}</td>
                                </tr>
                            @endforeach
